...
def load_model_and_vectorizer():
    with open('twitter_sentiment_analysis.pkl','rb') as model_file:
...
def create_card(tweet_text,sentiment):
    ...
    card_html =f"""
    <div style="background-color: {color}; padding: 1-px; boreder-radius: 5px; margin: 10px 0;">
        ...
        if st.button("Fetch Tweets"):
            ...
            if 'tweets' in tweets_data: #check if the 'tweets' key exists
                for tweet in tweets_data['tweets']:
                    ...
                    #create and display he colored card for the tweet
                    card_html = create_card(tweet_text,sentiment)
                    st.markdown(card_html,unsafe_allow_html=True)
            else:
                st.write("No tweets found or an error occured.")
